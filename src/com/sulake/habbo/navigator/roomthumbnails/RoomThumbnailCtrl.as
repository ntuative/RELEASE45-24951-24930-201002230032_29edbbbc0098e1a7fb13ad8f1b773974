package com.sulake.habbo.navigator.roomthumbnails
{
   ...
   public class RoomThumbnailCtrl
   {
      ...
      {
         setEnabled(_bgTabButton,this._model.mode != ThumbnailEditorModel.const_18);
         setEnabled(var_1429,this._model.mode != ThumbnailEditorModel.const_35);
         setEnabled(var_1428,this._model.mode != ThumbnailEditorModel.const_16);
      }
      ...
      private function onBgTabButton(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this._model.mode = ThumbnailEditorModel.const_18;
         this.reload();
      }
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         ...
         var_1429 = IButtonWindow(find("top_tab"));
         var_1428 = IButtonWindow(find("obj_tab"));
         var_1898 = IWindowContainer(find("tile_grid"));
         var_1897 = IWindowContainer(find("now_editing_container"));
         ...
         Util.setProcDirectly(var_1429,onTopTabButton);
         Util.setProcDirectly(var_1428,onObjTabButton);
      }
      ...
      private function refreshPositionGrid() : void
      {
         ...
         var _loc4_:* = null;
         var _loc5_:* = null;
         var _loc1_:IWindowContainer = var_1898;
         _loc1_.visible = true;
         if(_loc1_.numChildren == 0)
         {
            while(_loc3_ <= RoomThumbnailRenderer.const_331)
            {
      ...
      private function onObjTabButton(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this._model.mode = ThumbnailEditorModel.const_16;
         this.reload();
      }
      
      private function refreshEditArea() : void
      {
         var _loc1_:IWindowContainer = var_1897;
      ...
      private function populateImgList(param1:IItemListWindow, param2:int) : void
      {
         ...
         while(_loc8_ < _loc6_.length)
         {
            ...
            if(_loc9_ == 0)
            {
               _loc7_ = IWindowContainer(_navigator.windowManager.createWindow("row_" + _loc8_,"",HabboWindowType.const_37,HabboWindowStyle.const_20,HabboWindowParam.const_49,new Rectangle(0,0,param1.width,66)));
               param1.addListItem(_loc7_);
            }
            _loc10_ = _loc6_[_loc8_];
            _loc11_ = IBitmapWrapperWindow(_navigator.windowManager.createWindow("img_" + _loc8_,"",HabboWindowType.BITMAP_WRAPPER,HabboWindowStyle.const_23,0 | 0,new Rectangle(_loc9_ * 66,0,66,66),onImgSel,_loc10_.id));
      ...
      {
         refreshImgSelector(ThumbnailEditorModel.const_18);
         refreshImgSelector(ThumbnailEditorModel.const_35);
         refreshImgSelector(ThumbnailEditorModel.const_16);
      }
