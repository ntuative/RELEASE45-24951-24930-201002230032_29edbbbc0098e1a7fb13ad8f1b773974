package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      public static const const_265:int = 100;
      ...
      {
         return Math.ceil(this.var_201.length / const_265);
      }
      ...
      {
         if(this.var_742 >= this.getPageCount())
         {
            this.var_742 = Math.max(0,this.getPageCount() - 1);
         }
      }
      ...
      {
         var _loc4_:* = null;
         if(var_201.indexOf(param1) >= 0)
         {
            return;
         }
         var _loc2_:String = param1.name.toLowerCase();
         var _loc3_:int = 0;
         while(_loc3_ < var_201.length)
         {
            _loc4_ = var_201[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               var_201.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      {
         checkPageIndex();
         return Math.min((this.var_742 + 1) * const_265,this.var_201.length);
      }
