package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_454:BitmapData;
      
      private var var_1135:XML;
      
      private var var_275:IWindowContainer;
      
      private var var_378:IItemGridWindow;
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         var_378.destroyGridItems();
         var_275 = null;
         for each(_loc2_ in colors)
         {
            _loc3_ = param1.buildFromXML(var_1135) as IWindowContainer;
            _loc3_.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,onClick);
            _loc3_.background = true;
            _loc3_.color = 4294967295;
            _loc3_.width = var_454.width;
            _loc3_.height = var_454.height;
            var_378.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(var_454.width,var_454.height,true,0);
               _loc4_.bitmap.copyPixels(var_454,var_454.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(var_763.width,var_763.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(var_636.width,var_636.height,true,16777215);
               _loc6_.bitmap.copyPixels(var_636,var_636.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = var_378.getGridItemIndex(param1.target as IWindow);
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= var_378.numGridItems)
         {
            return;
         }
         select(var_378.getGridItemAt(param1) as IWindowContainer);
      }
      ...
      private function select(param1:IWindowContainer) : void
      {
         ...
         if(var_275 != null)
         {
            _loc2_ = var_275.getChildByName("chosen");
            if(_loc2_ != null)
            {
