package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_382 >= var_279.length)
         {
            return "";
         }
         var petColor:IPetColor = var_279[var_382] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_536[var_1406] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(var_382 < 0 || var_382 > var_279.length)
         {
            var_382 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_517.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_121,onTextWindowEvent);
         var_517 = new Map();
         ...
         var_780 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_279 = [];
         var_536 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_780);
               if(_loc11_)
               {
                  var_536 = _loc11_.breeds;
                  var_279 = _loc11_.colors;
                  if(var_279 == null)
                  {
                     var_279 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_536.length)
         {
            _loc12_ = var_536[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_780,_loc12_.id),getRaceLocalizationKey(var_780,_loc12_.id)));
            _loc6_++;
         }
         var_517.add(var_780,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_767,onApproveNameResult);
         _loc5_ = var_517.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_279)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_517.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_382 >= 0 && var_382 < var_279.length)
         {
            _loc7_ = var_279[var_382] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_536[var_1406] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_780,_loc3_.id,_loc2_,AvatarScaleType.const_33);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_300,AvatarAction.const_689);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_21,2);
         setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_21).clone());
         _loc4_.dispose();
