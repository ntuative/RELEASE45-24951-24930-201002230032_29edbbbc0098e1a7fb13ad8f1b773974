package com.sulake.core.window.utils
{
   ...
   public class WindowMouseOperator implements IDisposable
   {
      ...
      protected var var_111:DisplayObject;
      
      private var _disposed:Boolean = false;
      
      protected var var_1065:uint;
      ...
      protected function handler(param1:Event) : void
      {
         ...
         if(_working)
         {
            if(param1.type == Event.ENTER_FRAME)
            {
               ...
               {
                  operate(var_111.mouseX,var_111.mouseY);
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(_working)
         {
            if(_window == param1)
            {
               var_111.removeEventListener(MouseEvent.MOUSE_DOWN,handler,false);
               var_111.removeEventListener(MouseEvent.MOUSE_UP,handler,false);
               var_111.removeEventListener(Event.ENTER_FRAME,handler);
               if(true)
               {
                  _window.removeEventListener(WindowEvent.const_427,clientWindowDestroyed);
               }
               _window = null;
               _working = false;
            }
      ...
      public function begin(param1:IWindow, param2:uint = 0) : IWindow
      {
         ...
         {
            var_111.addEventListener(MouseEvent.MOUSE_DOWN,handler,false);
            var_111.addEventListener(MouseEvent.MOUSE_UP,handler,false);
            var_111.addEventListener(Event.ENTER_FRAME,handler);
            _mouse.x = var_111.mouseX;
            _mouse.y = var_111.mouseY;
            _window = WindowController(param1);
            getMousePositionRelativeTo(param1,_mouse,_offset);
            _window.addEventListener(WindowEvent.const_427,clientWindowDestroyed);
            _working = true;
         }
         return _loc3_;
      }
